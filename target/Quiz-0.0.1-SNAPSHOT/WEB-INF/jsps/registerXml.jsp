...
<%@page import="com.unt.quiz.Users"%>
<%@page import="com.unt.quiz.ResultModel"%>
<%@page import="com.unt.quiz.Helper"%>
  ...
  String sPageNo = request.getParameter("page");
	String sRowsPerPage = request.getParameter("rows");
		...
		if (count > 0 && nRowsPerPage != 0) {
  		...
  		StringBuilder sql =new StringBuilder("SELECT ID ,username ,email,mobile,city "+
  				" FROM  users");
...
<%   for (Users obj : list) {
	   ...
	   <cell><![CDATA[<%=obj.getCity()%>]]></cell>
	      <cell><![CDATA[<%="http://localhost:9090/Quiz/dowImg?id="+obj.getId()%>]]></cell>
</row>
    	   <%}
    	   %>
    	</rows>
